...
#define PAL_XY_OFFSET(xy, x, y)                                                                                        \
    (PAL_POS)(((((INT)(y) << 16) & 0xFFFF0000) + ((xy) & 0xFFFF0000)) | (((INT)(x) & 0xFFFF) + ((xy) & 0xFFFF)))
...
typedef enum tagMUSICTYPE { MUSIC_MIDI, MUSIC_RIX, MUSIC_MP3, MUSIC_OGG, MUSIC_OPUS } MUSICTYPE, *LPMUSICTYPE;

typedef enum tagCDTYPE { CD_NONE, CD_MP3, CD_OGG, CD_OPUS, CD_SDLCD } CDTYPE, *LPCDTYPE;

typedef enum tagMIDISYNTHTYPE { SYNTH_NATIVE, SYNTH_TIMIDITY, SYNTH_TINYSOUNDFONT } MIDISYNTHTYPE, *LPMIDISYNTHTYPE;
...
// 此处用到SDL库提供的函数，需要依照功能修改
INT PAL_RLEBlitToSurface(LPCBITMAPRLE lpBitmapRLE, /**SDL_Surface *lpDstSurface,**/ PAL_POS pos, bool bShadow);

INT PAL_RLEBlitToSurfaceWithShadow(LPCBITMAPRLE lpBitmapRLE,
                                   /**SDL_Surface *lpDstSurface,**/
                                   PAL_POS pos, bool bShadow);
...
INT PAL_FBPBlitToSurface(LPBYTE lpBitmapFBP
                         /**SDL_Surface *lpDstSurface,**/);
